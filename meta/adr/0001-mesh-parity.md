...
- **Collection naming**: the `ms_<name>_<hash>` derivation is reimplemented byte-for-byte from memsearch's plugin shell script (sanitized basename, 40-char cap, first 8 hex of SHA-256 of the absolute project path). It is not in the Python package, and we do not invent our own scheme. Like the stock plugins, pi passes it as a default that a global or project `[milvus].collection` outranks ([ADR 0008](0008-config-outranks-derived-collection.md)).
