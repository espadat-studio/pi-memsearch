...
**Collection** is a _default_, not a name pi imposes. pi derives `ms_<sanitized-basename>_<8 hex of sha256(abs path)>`, memsearch's derivation (`src/scope.ts`), and passes it as `--default-collection` on every `search`, `expand`, `compact`, `index` and `stats`. It never passes `-c` on this path. memsearch then layers it exactly as it does for every stock mesh plugin:

```
built-in default -> --default-collection (pi's derivation) -> ~/.memsearch/config.toml -> <repository dir>/.memsearch.toml -> -c
```

So a `[milvus].collection` in the global config outranks the derivation, and one in the project config outranks both. That is how several worktrees share one collection. Unpinned, the derivation wins and pi searches the same collection every other mesh agent builds. It hashes the absolute path: moving a repo yields a new collection, and the next `session_start` catch-up indexes into it.

The _resolved_ collection is what memsearch actually reads, and pi asks for it instead of assuming the derivation: `config get milvus.collection --default-collection <derived>`, run at the repository directory. `memory_status`, `memory_compact`'s result, and cross-repo dedup report and compare resolved names. The answer is memoized per directory and default name for the life of the process, so it costs one subprocess per directory. Indexing and single-project search need no answer at all, because the flag carries the layering. A config edited mid-session reaches every data operation at once, but the reported name only on the next process.

The store command is the exception. Its collection answer is passed as `-c`, memsearch's highest-priority source, so no config file can outrank it (see Store command).

The path it hashes is the project scope, `$MEMSEARCH_DIR` included. That is upstream's rule, not a side effect of pi deriving the store and the collection from one value: an explicit override means global scope, so the shared directory and the shared collection move together (`plugins/claude-code/hooks/common.sh:38-40,58-64`; memsearch [`41ccae5`](https://github.com/zilliztech/memsearch/commit/41ccae51ed6df2a3c07e598b89dc89e3e27366e5), shipped in v0.4.17). Keying it to the repository directory instead reads like the fix for a store/index split and is the split: it would break every stock plugin pairing (ADR 0001). Plugin checkouts older than 2026-04-30 show the pre-`41ccae5` shell, which keyed only to the project directory. [#100](https://github.com/sripwoud/pi-memsearch/issues/100) was filed off one and closed on this.
...
- Fans out across every project found under `PI_MEMSEARCH_SCAN_ROOTS`: one sequential search per project, including projects only other mesh agents ever indexed. The current project keeps its `--default-collection`; every discovered project is searched as `-c <resolved name>`. memsearch children run at the session's repository directory, so a default passed for a foreign project would be outranked by the _session's_ project config.
...
- Never-indexed projects are skipped and counted. Distinct projects resolving to one collection are _collapsed_, keyed on resolved names, so two projects pinned by config to one collection are searched once: the first is searched, the rest are listed by path. Reaching the same directory twice (the current project sitting under a scan root) is still deduplicated silently. The collapsed count joins the accounting line only when there is one.
...
Default recall stays project-scoped, and no store or collection is ever written. There is deliberately no global store ([ADR 0003](https://github.com/espadat-studio/pi-memsearch/blob/master/meta/adr/0003-no-global-store-cross-repo-recall.md)).
...
Opt-in via `PI_MEMSEARCH_AUTO_CONTEXT=on`. Every user prompt runs a semantic search over the project collection and injects the hits as a message invisible in the TUI. The sidecar receives the collection as the CLI does: a derived name as `default_collection`, layered under the global and project config with memsearch's own `resolve_config(default_overrides=…)` at the repository directory, and a store-command name as `collection`, used as is. Resolving costs no subprocess per prompt. Design and rejected alternatives: [ADR 0005](https://github.com/espadat-studio/pi-memsearch/blob/master/meta/adr/0005-per-session-sidecar-auto-context.md).
...
`memory_status` reports `collection: unresolved (<reason>; default <derived>)` when the resolved name cannot be read, including when the backend is unavailable. An aborted call still aborts. `memory_compact` and cross-repo search fail instead, since they need the name.
